...
public class IapDriftMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        MiningGate.register();
        DeniedOutputPurger.register();
        IapDriftCommands.register();
        LOG.info("[IAP-Drift] Loaded config with {} tasks; enabled={}.", config.tasks.size(), config.enabledTaskCount());
    }

    public static DriftConfig config() { return config; }
    public static DriftLogger truthLogger() { return truthLogger; }

    public static void reloadConfig() {
        config = DriftConfig.loadOrCreate();
        if (truthLogger == null) truthLogger = new DriftLogger(config.truth_log_file);
        LOG.info("[IAP-Drift] Reloaded config with {} tasks; enabled={}.", config.tasks.size(), config.enabledTaskCount());
    }
}
